   ...
   */
	...
	before             : function(scope, cb) {
		...
		attributes = _.map(attributes, function(attribute, i) {
			...
			if (!parts[1] || !parts[0]) {
				invalidAttributes.push('Invalid attribute notation:   "' + attribute + '"');
				return;
			}
			return {
				name : parts[0],
...
**/
		...
		var ACTION_UPDATE_PARAM_OBJECT_TEMPLATE = path.resolve(
			__dirname,
			'./templates/actionUpdateParamObject.template'
		);
   ...
   */
	...
	targets            : {
		...
		'./assets/styles/custom.css'            : {
			template : { templatePath: 'customCSS.template', force: true }
		},

		'./assets/styles/bootstrapScaffold.css' : {
			template : { templatePath: 'bootstrapScaffoldCSS.template', force: true }
		},

		'./views/pages/scaffold.ejs'            : {
			template : { templatePath: 'homePage.template', force: true }
		},

		'./views/:viewDirectoryName/new.ejs'    : {
			template : { templatePath: 'new.template', force: true }
		},

		'./views/:viewDirectoryName/show.ejs'   : {
			template : { templatePath: 'show.template', force: true }
		},

		'./views/:viewDirectoryName/index.ejs'  : {
			template : { templatePath: 'index.template', force: true }
		},

		'./views/:viewDirectoryName/edit.ejs'   : {
			template : { templatePath: 'edit.template', force: true }
		},

		'./api/policies/flash.js'               : {
			template : { templatePath: 'flashPolicy.template', force: true }
		},

		'./config/scaffold-policies.js'         : {
			template : { templatePath: 'policies.template', force: true }
		}
	},
